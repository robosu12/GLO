...
class CPUMetric {
 public:
  CPUMetric(size_t pid = process_id())
      : id(pid), last_total(get_cpu_total_occupy()), last_proc(get_cpu_proc_occupy(pid)) {}

  float usage() {
    unsigned long total = get_cpu_total_occupy();
    unsigned long proc = get_cpu_proc_occupy(id);
    if (proc - last_proc < 100)
      return last_usage;
    last_usage =
        std::thread::hardware_concurrency() * 100. * (proc - last_proc) / (total - last_total);
    last_proc = proc;
    last_total = total;
    return last_usage;
  }

  static size_t process_id() { return getpid(); }
